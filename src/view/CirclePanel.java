...
public class CirclePanel extends JPanel {
	...
	public CirclePanel(int player, int timeInAll){
		this.player = player;
		this.timeInAll = timeInAll;
		this.timeRest = timeInAll;
		
		logo = Images.PLAYER_LOGO[player];
		playerLogo = new JLabel();
		playerLogo.setIcon(logo);
		playerLogo.setBackground(Color.green);
	...
	public void paintComponent(Graphics g){
		...
		//画扇形
		Stroke strokeZero = new BasicStroke(0);
		g2.setStroke(strokeZero);
		...
		startAngle = 90.0;
		arcAngle = 180.0 / timeInAll;
		g2.setColor(BlockColor.getOtherColor(1));
