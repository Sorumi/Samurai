...
public class GamePanel extends Pane implements Observer {
	...
	public OrderPanel samuraiActionPanel;
	...
	/*
	 * level: 0: 双人 99:经典 11-53:故事 -1:自定义
	 */
	public GamePanel(int size, int level) {
		...
		if (level < 99 && level > 0) {
			orderList = FXCollections.observableArrayList(backgroundPanel, chessBoard, A1, A2, A3, B1, B2, B3,
					samuraiActionPanel, arrow, actionPanel, statePanel, playerA, playerB, roundPanel, systemPanel,
					resultPanel, propPanel, propsGroup, selectPanel, overlayPanel, circleLight, pausePanel);
		} else if (level == -1) {
			orderList = FXCollections.observableArrayList(backgroundPanel, chessBoard, A1, A2, A3, B1, B2, B3,
					samuraiActionPanel, arrow, actionPanel, statePanel, playerA, playerB, roundPanel, systemPanel,
					selectPanel, pausePanel);
		} else {
			orderList = FXCollections.observableArrayList(backgroundPanel, chessBoard, A1, A2, A3, B1, B2, B3,
					samuraiActionPanel, arrow, actionPanel, playerA, playerB, roundPanel, systemPanel, selectPanel,
					pausePanel);
		}
		this.setOrder();
					...
					public void run() {
						...
						if (canAction) {
							...
							if (currentPlayer == playerA) {
								...
								samuraiActionPanel.setLayoutX(currentSamurai.getLayoutX());
								samuraiActionPanel.setLayoutY(currentSamurai.getLayoutY());
								samuraiActionPanel.setVisible(true);
								resetEvents();
							}
							...
							samuraiActionPanel.setOnMouseEntered(null);
							samuraiActionPanel.setVisible(false);
						}
					}
				});
			}
	...
	public void setBackground(int level) {
		this.setStyle(
				"-fx-background-image: url(" + Images.BG[level / 10] + ") ;" + "-fx-background-size: 100% 100%; ");
		switch (level / 10) {
		case 0:
			backgroundPanel = new BackgroundPanel0();
			break;
		case 1:
			backgroundPanel = new BackgroundPanel1();
			break;
		case 2:
			backgroundPanel = new BackgroundPanel2();
			break;
		case 3:
			backgroundPanel = new BackgroundPanel3();
			break;
		case 4:
			backgroundPanel = new BackgroundPanel4();
			break;
		case 5:
			backgroundPanel = new BackgroundPanel5();
			break;
		default:
			backgroundPanel = new BackgroundPanel0();
		}
	...
	public void setCurrentSamurai(int num) {
		...
		currentSamurai
				.setCanHide(chessBoard.getState(currentSamurai.x, currentSamurai.y) / 4 == currentSamurai.getNum() / 4);
		...
		Platform.runLater(new Runnable() {
			public void run() {
				if (key.equals("samurai")) {
					...
					currentSamurai.setCanHide(
							chessBoard.getState(currentSamurai.x, currentSamurai.y) / 4 == currentSamurai.getNum() / 4);
					...
					if (currentPlayer.getPlayer() == 0) {
						currentSamurai.setCanHide(chessBoard.getState(currentSamurai.x, currentSamurai.y)
								/ 4 == currentSamurai.getNum() / 4);
						actionPanel.reset();
					...
					int[] t = (int[]) notifingObject.getValue();
					// System.out.println("Position : " + t[0] + " , " + t[1] +
					// " get prop ");
